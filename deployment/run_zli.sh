#!/usr/bin/env bash
# Launcher for run_zli.py.
#
# Same X11/xcb workarounds as run_wm_only.sh (uv-managed CPython + NVIDIA X11):
...
#   segfaulting glfw/OpenCV and deadlocking the NVIDIA Vulkan ICD during
#   SAPIEN Device() creation.  Preloading the real system libs fixes both;
...
#
# usage: ./run_zli.sh [args for run_zli.py...]
set -euo pipefail
...
exec "$SCRIPT_DIR/../.venv/bin/python" -u "$SCRIPT_DIR/run_zli.py" "$@"
